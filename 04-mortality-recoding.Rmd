...
library(readr)
ICD10list3d <- read_delim("icd10_coding19.tsv", 
    "\t", 
    escape_double = FALSE, 
    trim_ws = TRUE)
...
# date of death data
dat_long <- read_delim("updated_mortality_july_21.txt",
    "\t", 
    escape_double = FALSE, 
    trim_ws = TRUE)

# cause of death (COD) data. More info here: http://biobank.ndph.ox.ac.uk/showcase/showcase/docs/DeathLinkage.pdf.
dat_COD <- read_delim("updated_mortality_cause_july_21.txt", # cause of death table. 
    "\t", 
    escape_double = FALSE, 
    trim_ws = TRUE)
...
charlson <- readRDS("ukb_charlson_comorbidity_index_baseline.rds")
...
saveRDS(long, file = "ukb_mortality_and comorbidity.rds") # save RDS to file

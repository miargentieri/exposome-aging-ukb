...
df_charlson <- subset(dat, select = c("f.eid", 
                                          ...
                                          ICD_baseline,
                                          ICD10_all_date))
# save before baseline ICD10 info
saveRDS(df_charlson, 
        file = "/Users/austin.argentieri/OneDrive - Nexus365/DPhil Research/UKB/Datasets/4. Categorized raw datasets/ukb_ICD10_codes_before_baseline.rds")  

```

```{r, plain.ascii = FALSE, results='asis', include=FALSE}


# save ICD codes across all times (before + after baseline)
df_ICD <- subset(dat, select = c("f.eid",
                                     "f.53.0.0",
                                     ICD10_all,
                                     ICD10_all_date))

saveRDS(df_ICD, 
        file = "/Users/austin.argentieri/OneDrive - Nexus365/DPhil Research/UKB/Datasets/4. Categorized raw datasets/ukb_ICD10_codes_all_years.rds") # save RDS to file 


# Identify follow up ICD10 codes (diagnosed after recruitment date)

# here is the loop
for(j in seq_along(icd_follow)){
  dat[[icd_follow[j]]] <- ifelse(dat[[ICD10_all_date[j]]] > dat$f.53.0.0, # if ICD10 diagnosis date is after recruitment
                                     dat[[ICD10_all[j]]], # keep corresponding ICD10 code
                                     NA) # or else NA
}

# create df with the new ICD10 vars after baseline
df_charlson_follow_up <- subset(dat, select = c("f.eid",
                                                    "f.53.0.0",
                                                    icd_follow,
                                                    ICD10_main_date))

saveRDS(df_charlson_follow_up, 
        file = "/Users/austin.argentieri/OneDrive - Nexus365/DPhil Research/UKB/Datasets/4. Categorized raw datasets/ukb_ICD10_codes_after_baseline.rds") # save RDS to file 
```

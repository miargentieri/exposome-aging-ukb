...
# load raw dataset with all exposures

dat <- readRDS("/Users/austin.argentieri/OneDrive - Nexus365/DPhil Research/UKB/Datasets/4. Categorized raw datasets/ukb_all_exposures_updated.rds")

library(Hmisc)
library(summarytools)
library(dplyr)

# remove participants who requested to be removed (eids sent in Aug 23, 2020 email from UKB)
eid_remove <- c(
  Cs(
    1118928,
    1189363,
...
dat <- dat[dat$eid %nin% eid_remove, ]
...
unique(dat$country_birth_non_UK)
unique(dat$childhood_love)
unique(dat$childhood_physical_abuse)
unique(dat$childhood_hate)
unique(dat$childhood_sex_abuse)
unique(dat$childhood_doctor)
...
# dfSummary(dat$body_size_10yrs_old)
...
# dfSummary(dat$height_10yrs_old)
...
dat$country_birth_non_UK <- as.factor(dat$country_birth_non_UK)

dat$childhood_love <- factor(dat$childhood_love, ordered = FALSE)
dat$childhood_love <- relevel(dat$childhood_love, ref = "Never true")

dat$childhood_physical_abuse <- factor(dat$childhood_physical_abuse, ordered = FALSE)
dat$childhood_physical_abuse <- relevel(dat$childhood_physical_abuse, ref = "Never true")

dat$childhood_hate <- factor(dat$childhood_hate, ordered = FALSE)
dat$childhood_hate <- relevel(dat$childhood_hate, ref = "Never true")

dat$childhood_sex_abuse <- factor(dat$childhood_sex_abuse, ordered = FALSE)
dat$childhood_sex_abuse <- relevel(dat$childhood_sex_abuse, ref = "Never true")

dat$childhood_doctor <- factor(dat$childhood_doctor, ordered = FALSE)
dat$childhood_doctor <- relevel(dat$childhood_doctor, ref = "Never true")
...
dat$relative_illness_injury_assault <- ifelse(rowSums(dat[f] == "Serious illness, injury or assault of a close relative", na.rm = TRUE) > 0, 1, 
                                              ifelse(rowSums(is.na(dat[f])) < length(f), 0, NA))
...
dat$death_relative <- ifelse(rowSums(dat[f] == "Death of a close relative", na.rm = TRUE) > 0, 1,
                             ifelse(rowSums(is.na(dat[f])) < length(f), 0, NA))
...
dfSummary(dat$accommodation_type) # check to see most frequent level
...
dfSummary(dat$own_or_rent) # check to see most frequent level
...
dat$years_at_address[dat$years_at_address == "-10"] <- 0
dat$years_at_address[dat$years_at_address == "-3"] <- NA
dat$years_at_address[dat$years_at_address == "-1"] <- NA

dat$hshld_number <- as.numeric(dat$hshld_number)
dat$hshld_number[dat$hshld_number == "-3"] <- NA
dat$hshld_number[dat$hshld_number == "-1"] <- NA
...
dat$education_age_complete[dat$education_age_complete == "-3"] <- NA
dat$education_age_complete[dat$education_age_complete == "-2"] <- NA
dat$education_age_complete[dat$education_age_complete == "-1"] <- NA

dfSummary(dat$employment_corrected) # check to see most frequent level
...
dat$employment_distance[dat$employment_distance == "-10"] <- 0
dat$employment_distance[dat$employment_distance == "-3"] <- NA
dat$employment_distance[dat$employment_distance == "-1"] <- NA

dat$employment_years <- as.numeric(dat$employment_years)
dat$employment_years[dat$employment_years == "-10"] <- 0
dat$employment_years[dat$employment_years == "-3"] <- NA
dat$employment_years[dat$employment_years == "-1"] <- NA

dat$employment_hours <- as.numeric(dat$employment_hours)
dat$employment_hours[dat$employment_hours == "-3"] <- NA
dat$employment_hours[dat$employment_hours == "-1"] <- NA

dat$employment_travel_freq <- as.numeric(dat$employment_travel_freq)
dat$employment_travel_freq[dat$employment_travel_freq == "-10"] <- 0
dat$employment_travel_freq[dat$employment_travel_freq == "-3"] <- NA
dat$employment_travel_freq[dat$employment_travel_freq == "-1"] <- NA
...
dat$immigration_year[dat$immigration_year == "-3"] <- NA
dat$immigration_year[dat$immigration_year == "-1"] <- NA

dfSummary(dat$ethnicity) # check to see most frequent level
...
dfSummary(dat$sex) # check to see most frequent level
...
dat$father_age[dat$father_age == "-3"] <- NA
dat$father_age[dat$father_age == "-1"] <- NA

dat$father_age_death <- as.numeric(dat$father_age_death)
dat$father_age_death[dat$father_age_death == "-3"] <- NA
dat$father_age_death[dat$father_age_death == "-1"] <- NA
...
dat$mother_age[dat$mother_age == "-3"] <- NA
dat$mother_age[dat$mother_age == "-1"] <- NA

dat$mother_age_death <- as.numeric(dat$mother_age_death)
dat$mother_age_death[dat$mother_age_death == "-3"] <- NA
dat$mother_age_death[dat$mother_age_death == "-1"] <- NA

dat$number_brothers <- as.numeric(dat$number_brothers)
dat$number_brothers[dat$number_brothers == "-3"] <- NA
dat$number_brothers[dat$number_brothers == "-1"] <- NA

dat$number_adopted_brothers <- as.numeric(dat$number_adopted_brothers)
dat$number_adopted_brothers[dat$number_adopted_brothers == "-3"] <- NA
dat$number_adopted_brothers[dat$number_adopted_brothers == "-1"] <- NA

dat$number_sisters <- as.numeric(dat$number_sisters)
dat$number_sisters[dat$number_sisters == "-3"] <- NA
dat$number_sisters[dat$number_sisters == "-1"] <- NA

dat$number_adopted_sisters <- as.numeric(dat$number_adopted_sisters)
dat$number_adopted_sisters[dat$number_adopted_sisters == "-3"] <- NA
dat$number_adopted_sisters[dat$number_adopted_sisters == "-1"] <- NA

dat$number_older_siblings <- as.numeric(dat$number_older_siblings)
dat$number_older_siblings[dat$number_older_siblings == "-3"] <- NA
dat$number_older_siblings[dat$number_older_siblings == "-1"] <- NA
...
dat$MI_diagnosis_age[dat$MI_diagnosis_age == "-3"] <- NA         
dat$MI_diagnosis_age[dat$MI_diagnosis_age == "-1"] <- NA               

dat$angina_diagnosis_age <- as.numeric(dat$angina_diagnosis_age)
dat$angina_diagnosis_age[dat$angina_diagnosis_age == "-3"] <- NA         
dat$angina_diagnosis_age[dat$angina_diagnosis_age == "-1"] <- NA               

dat$stroke_diagnosis_age <- as.numeric(dat$stroke_diagnosis_age)
dat$stroke_diagnosis_age[dat$stroke_diagnosis_age == "-3"] <- NA         
dat$stroke_diagnosis_age[dat$stroke_diagnosis_age == "-1"] <- NA               

dat$bp_diagnosis_age <- as.numeric(dat$bp_diagnosis_age)
dat$bp_diagnosis_age[dat$bp_diagnosis_age == "-3"] <- NA         
dat$bp_diagnosis_age[dat$bp_diagnosis_age == "-1"] <- NA               

dat$DVT_diagnosis_age <- as.numeric(dat$DVT_diagnosis_age)
dat$DVT_diagnosis_age[dat$DVT_diagnosis_age == "-3"] <- NA         
dat$DVT_diagnosis_age[dat$DVT_diagnosis_age == "-1"] <- NA               

dat$pulm_embolism_diagnosis_age <- as.numeric(dat$pulm_embolism_diagnosis_age)
dat$pulm_embolism_diagnosis_age[dat$pulm_embolism_diagnosis_age == "-3"] <- NA         
dat$pulm_embolism_diagnosis_age[dat$pulm_embolism_diagnosis_age == "-1"] <- NA               

dat$emphysema_diagnosis_age <- as.numeric(dat$emphysema_diagnosis_age)
dat$emphysema_diagnosis_age[dat$emphysema_diagnosis_age == "-3"] <- NA         
dat$emphysema_diagnosis_age[dat$emphysema_diagnosis_age == "-1"] <- NA               

dat$asthma_diagnosis_age <- as.numeric(dat$asthma_diagnosis_age)
dat$asthma_diagnosis_age[dat$asthma_diagnosis_age == "-3"] <- NA         
dat$asthma_diagnosis_age[dat$asthma_diagnosis_age == "-1"] <- NA               

dat$rhinitis_diagnosis_age <- as.numeric(dat$rhinitis_diagnosis_age)
dat$rhinitis_diagnosis_age[dat$rhinitis_diagnosis_age == "-3"] <- NA         
dat$rhinitis_diagnosis_age[dat$rhinitis_diagnosis_age == "-1"] <- NA               
...
dat$diabetes_diagnosis_age[dat$diabetes_diagnosis_age == "-3"] <- NA         
dat$diabetes_diagnosis_age[dat$diabetes_diagnosis_age == "-1"] <- NA               
...
dat$glasses_age[dat$glasses_age == "-3"] <- NA         
dat$glasses_age[dat$glasses_age == "-1"] <- NA               

eye_first <- match("myopia_eye",names(dat)) # col #
eye_last <- match("other_serious_eye",names(dat)) # col #
dfSummary(dat[eye_first:eye_last]) # check to see the most common response to set as the ref level
...
dfSummary(dat[eye_first2:eye_last2]) # check to see the most common response to set as the ref level
...
dat$diabetes_eye_age[dat$diabetes_eye_age == "-3"] <- NA         
dat$diabetes_eye_age[dat$diabetes_eye_age == "-1"] <- NA      

dat$glaucoma_age <- as.numeric(dat$glaucoma_age)
dat$glaucoma_age[dat$glaucoma_age == "-3"] <- NA         
dat$glaucoma_age[dat$glaucoma_age == "-1"] <- NA               

dat$injury_eye_age <- as.numeric(dat$injury_eye_age)
dat$injury_eye_age[dat$injury_eye_age == "-3"] <- NA         
dat$injury_eye_age[dat$injury_eye_age == "-1"] <- NA               

dat$cataract_age <- as.numeric(dat$cataract_age)
dat$cataract_age[dat$cataract_age == "-3"] <- NA         
dat$cataract_age[dat$cataract_age == "-1"] <- NA               

dat$macular_degeneration_age <- as.numeric(dat$macular_degeneration_age)
dat$macular_degeneration_age[dat$macular_degeneration_age == "-3"] <- NA         
dat$macular_degeneration_age[dat$macular_degeneration_age == "-1"] <- NA               

dat$other_serious_eye_age <- as.numeric(dat$other_serious_eye_age)
dat$other_serious_eye_age[dat$other_serious_eye_age == "-3"] <- NA         
dat$other_serious_eye_age[dat$other_serious_eye_age == "-1"] <- NA               
...
dfSummary(dat$leg_pain_walking_action) # check to see the most common response to set as the ref level
...
dfSummary(dat$leg_pain_walking_standing) # check to see the most common response to set as the ref level
...
dfSummary(dat$chest_pain_walking_normal) # check to see the most common response to set as the ref level
...
dat$bowel_cancer_screening_recent[dat$bowel_cancer_screening_recent == "-10"] <- 0         
dat$bowel_cancer_screening_recent[dat$bowel_cancer_screening_recent == "-3"] <- NA         
dat$bowel_cancer_screening_recent[dat$bowel_cancer_screening_recent == "-1"] <- NA               
...
dat$PSA_test_time_since[dat$PSA_test_time_since == "-10"] <- 0         
dat$PSA_test_time_since[dat$PSA_test_time_since == "-3"] <- NA         
dat$PSA_test_time_since[dat$PSA_test_time_since == "-1"] <- NA               
...
dat$cholesterol_insul <- recode_factor(dat$cholesterol_meds_v2,
                                         "0" = "No",
                                         "1" = "Yes")
...
dfSummary(dat$age_facial_hair) # check to see the most common response to set as the ref level
...
dfSummary(dat$age_voice) # check to see the most common response to set as the ref level
dat$age_voice <- factor(dat$age_voice, ordered = FALSE)
dat$age_voice <- relevel(dat$age_voice, ref = "About average age")

dfSummary(dat$balding) # check to see the most common response to set as the ref level
...
dat$number_children[dat$number_children == "-3"] <- NA         
dat$number_children[dat$number_children == "-1"] <- NA   
...
dat$mammogram_years_since[dat$mammogram_years_since == "-10"] <- 0      
dat$mammogram_years_since[dat$mammogram_years_since == "-3"] <- NA         
dat$mammogram_years_since[dat$mammogram_years_since == "-1"] <- NA  
...
dat$cervical_smear_years_since[dat$cervical_smear_years_since == "-10"] <- 0      
dat$cervical_smear_years_since[dat$cervical_smear_years_since == "-3"] <- NA         
dat$cervical_smear_years_since[dat$cervical_smear_years_since == "-1"] <- NA   

dat$menarche_age <- as.numeric(dat$menarche_age)
dat$menarche_age[dat$menarche_age == "-3"] <- NA         
dat$menarche_age[dat$menarche_age == "-1"] <- NA 

dat$menopause[dat$menopause == "Not sure - had a hysterectomy"] <- NA         
dat$menopause[dat$menopause == "Not sure - other reason"] <- NA  
...
dat$period_time_since[dat$period_time_since == "-3"] <- NA         
dat$period_time_since[dat$period_time_since == "-1"] <- NA 

dat$menstrual_cycle_length <- as.numeric(dat$menstrual_cycle_length)
dat$menstrual_cycle_length[dat$menstrual_cycle_length == "-6"] <- NA      
dat$menstrual_cycle_length[dat$menstrual_cycle_length == "-3"] <- NA         
dat$menstrual_cycle_length[dat$menstrual_cycle_length == "-1"] <- NA 
...
dat$number_births[dat$number_births == "-3"] <- NA         

dat$first_child_birth_weight <- as.numeric(dat$first_child_birth_weight)
dat$first_child_birth_weight[dat$first_child_birth_weight == "-3"] <- NA      
dat$first_child_birth_weight[dat$first_child_birth_weight == "-2"] <- NA         
dat$first_child_birth_weight[dat$first_child_birth_weight == "-1"] <- NA 

dat$birth_age <- as.numeric(dat$birth_age)
dat$birth_age[dat$birth_age == "-4"] <- NA      
dat$birth_age[dat$birth_age == "-3"] <- NA 

dat$first_birth_age <- as.numeric(dat$first_birth_age)
dat$first_birth_age[dat$first_birth_age == "-4"] <- NA      
dat$first_birth_age[dat$first_birth_age == "-3"] <- NA 

dat$last_birth_age <- as.numeric(dat$last_birth_age)
dat$last_birth_age[dat$last_birth_age == "-4"] <- NA      
dat$last_birth_age[dat$last_birth_age == "-3"] <- NA 
...
dat$number_stillbirths[dat$number_stillbirths == "-3"] <- NA         
dat$number_stillbirths[dat$number_stillbirths == "-1"] <- NA 

dat$number_miscarriages <- as.numeric(dat$number_miscarriages)
dat$number_miscarriages[dat$number_miscarriages == "-3"] <- NA         
dat$number_miscarriages[dat$number_miscarriages == "-1"] <- NA 

dat$number_pregnancy_terminations <- as.numeric(dat$number_pregnancy_terminations)
dat$number_pregnancy_terminations[dat$number_pregnancy_terminations == "-3"] <- NA         
dat$number_pregnancy_terminations[dat$number_pregnancy_terminations == "-1"] <- NA 
...
dat$oral_contraceptive_age_first[dat$oral_contraceptive_age_first == "-3"] <- NA         
dat$oral_contraceptive_age_first[dat$oral_contraceptive_age_first == "-1"] <- NA 

dat$oral_contraceptive_age_last <- as.numeric(dat$oral_contraceptive_age_last)
dat$oral_contraceptive_age_last[dat$oral_contraceptive_age_last == "-11"] <- NA         
dat$oral_contraceptive_age_last[dat$oral_contraceptive_age_last == "-3"] <- NA         
dat$oral_contraceptive_age_last[dat$oral_contraceptive_age_last == "-1"] <- NA 

dat$HRT <- factor(dat$HRT, ordered = FALSE)
dat$HRT <- relevel(dat$HRT, ref = "No")

dat$HRT_age_first <- as.numeric(dat$HRT_age_first)
dat$HRT_age_first[dat$HRT_age_first == "-3"] <- NA         
dat$HRT_age_first[dat$HRT_age_first == "-1"] <- NA 

dat$HRT_age_last <- as.numeric(dat$HRT_age_last)
dat$HRT_age_last[dat$HRT_age_last == "-11"] <- NA         
dat$HRT_age_last[dat$HRT_age_last == "-3"] <- NA         
dat$HRT_age_last[dat$HRT_age_last == "-1"] <- NA 

dat$hysterectomy[dat$hysterectomy == "Not sure"] <- NA  
...
dat$hysterectomy_age[dat$hysterectomy_age == "-3"] <- NA         
dat$hysterectomy_age[dat$hysterectomy_age == "-1"] <- NA 

dat$bilateral_oophorectomy[dat$bilateral_oophorectomy == "Not sure"] <- NA  
...
dat$bilateral_oophorectomy_age[dat$bilateral_oophorectomy_age == "-3"] <- NA         
dat$bilateral_oophorectomy_age[dat$bilateral_oophorectomy_age == "-1"] <- NA 

dat$pregnant[dat$pregnant == "Unsure"] <- NA  
...
dat$depression_episodes[dat$depression_episodes == "-3"] <- NA         
dat$depression_duration[dat$depression_duration == "-1"] <- NA 

dat$depression_episodes <- as.numeric(dat$depression_episodes)
dat$depression_episodes[dat$depression_episodes == "-3"] <- NA         
dat$depression_episodes[dat$depression_episodes == "-1"] <- NA 
...
dat$unenthusiastic_duration[dat$unenthusiastic_duration == "-3"] <- NA         
dat$unenthusiastic_duration[dat$unenthusiastic_duration == "-1"] <- NA 

dat$unenthusiastic_episodes <- as.numeric(dat$unenthusiastic_episodes)
dat$unenthusiastic_episodes[dat$unenthusiastic_episodes == "-3"] <- NA         
dat$unenthusiastic_episodes[dat$unenthusiastic_episodes == "-1"] <- NA 
...
dat$smoking_start_age_current[dat$smoking_start_age_current == "-3"] <- NA         
dat$smoking_start_age_current[dat$smoking_start_age_current == "-1"] <- NA 

dfSummary(dat$tobacco_type)
...
dat$number_cigarettes[dat$number_cigarettes == "-10"] <- 0         
dat$number_cigarettes[dat$number_cigarettes == "-3"] <- NA         
dat$number_cigarettes[dat$number_cigarettes == "-1"] <- NA 

dat$cigarettes_stop_age <- as.numeric(dat$cigarettes_stop_age)
dat$cigarettes_stop_age[dat$cigarettes_stop_age == "-3"] <- NA         
dat$cigarettes_stop_age[dat$cigarettes_stop_age == "-1"] <- NA 

dat$previous_number_cigarettes <- as.numeric(dat$previous_number_cigarettes)
dat$previous_number_cigarettes[dat$previous_number_cigarettes == "-10"] <- 0         
dat$previous_number_cigarettes[dat$previous_number_cigarettes == "-1"] <- NA 
...
dat$smoking_start_age_former[dat$smoking_start_age_former == "-3"] <- NA         
dat$smoking_start_age_former[dat$smoking_start_age_former == "-1"] <- NA 

dfSummary(dat$tobacco_type_previous)
...
dat$number_cigarettes_previous[dat$number_cigarettes_previous == "-10"] <- 0         
dat$number_cigarettes_previous[dat$number_cigarettes_previous == "-1"] <- NA 

dat$smoking_stop_age <- as.numeric(dat$smoking_stop_age)
dat$smoking_stop_age[dat$smoking_stop_age == "-3"] <- NA         
dat$smoking_stop_age[dat$smoking_stop_age == "-1"] <- NA 
...
dat$number_quit_attempts[dat$number_quit_attempts == "-3"] <- NA         
dat$number_quit_attempts[dat$number_quit_attempts == "-1"] <- NA 

dfSummary(dat$hshld_smokers)
...
dat$tobacco_exposure_home[dat$tobacco_exposure_home == "-3"] <- NA         
dat$tobacco_exposure_home[dat$tobacco_exposure_home == "-1"] <- NA 

dat$tobacco_exposure_outside <- as.numeric(dat$tobacco_exposure_outside)
dat$tobacco_exposure_outside[dat$tobacco_exposure_outside == "-3"] <- NA         
dat$tobacco_exposure_outside[dat$tobacco_exposure_outside == "-1"] <- NA 
...
dat$red_wine_monthly[dat$red_wine_monthly == "-3"] <- NA         
dat$red_wine_monthly[dat$red_wine_monthly == "-1"] <- NA 

dat$white_wine_monthly <- as.numeric(dat$white_wine_monthly)
dat$white_wine_monthly[dat$white_wine_monthly == "-3"] <- NA         
dat$white_wine_monthly[dat$white_wine_monthly == "-1"] <- NA 

dat$beer_monthly <- as.numeric(dat$beer_monthly)
dat$beer_monthly[dat$beer_monthly == "-3"] <- NA         
dat$beer_monthly[dat$beer_monthly == "-1"] <- NA 

dat$spirits_monthly <- as.numeric(dat$spirits_monthly)
dat$spirits_monthly[dat$spirits_monthly == "-3"] <- NA         
dat$spirits_monthly[dat$spirits_monthly == "-1"] <- NA 

dat$fortified_wine_monthly <- as.numeric(dat$fortified_wine_monthly)
dat$fortified_wine_monthly[dat$fortified_wine_monthly == "-3"] <- NA         
dat$fortified_wine_monthly[dat$fortified_wine_monthly == "-1"] <- NA 

dat$other_alcohol_monthly <- as.numeric(dat$other_alcohol_monthly)
dat$other_alcohol_monthly[dat$other_alcohol_monthly == "-3"] <- NA         
dat$other_alcohol_monthly[dat$other_alcohol_monthly == "-1"] <- NA 

dat$red_wine_weekly <- as.numeric(dat$red_wine_weekly)
dat$red_wine_weekly[dat$red_wine_weekly == "-3"] <- NA         
dat$red_wine_weekly[dat$red_wine_weekly == "-1"] <- NA 

dat$white_wine_weekly <- as.numeric(dat$white_wine_weekly)
dat$white_wine_weekly[dat$white_wine_weekly == "-3"] <- NA         
dat$white_wine_weekly[dat$white_wine_weekly == "-1"] <- NA 

dat$beer_weekly <- as.numeric(dat$beer_weekly)
dat$beer_weekly[dat$beer_weekly == "-3"] <- NA         
dat$beer_weekly[dat$beer_weekly == "-1"] <- NA 

dat$spirits_weekly <- as.numeric(dat$spirits_weekly)
dat$spirits_weekly[dat$spirits_weekly == "-3"] <- NA         
dat$spirits_weekly[dat$spirits_weekly == "-1"] <- NA 

dat$fortified_wine_weekly <- as.numeric(dat$fortified_wine_weekly)
dat$fortified_wine_weekly[dat$fortified_wine_weekly == "-3"] <- NA         
dat$fortified_wine_weekly[dat$fortified_wine_weekly == "-1"] <- NA 

dat$other_alcohol_weekly <- as.numeric(dat$other_alcohol_weekly)
dat$other_alcohol_weekly[dat$other_alcohol_weekly == "-3"] <- NA         
dat$other_alcohol_weekly[dat$other_alcohol_weekly == "-1"] <- NA 
...
dfSummary(dat$reason_stopped_alcohol)
...
dat$cooked_veg[dat$cooked_veg == "-10"] <- 0         
dat$cooked_veg[dat$cooked_veg == "-3"] <- NA         
dat$cooked_veg[dat$cooked_veg == "-1"] <- NA 

dat$salad_or_raw_veg <- as.numeric(dat$salad_or_raw_veg)
dat$salad_or_raw_veg[dat$salad_or_raw_veg == "-10"] <- 0         
dat$salad_or_raw_veg[dat$salad_or_raw_veg == "-3"] <- NA         
dat$salad_or_raw_veg[dat$salad_or_raw_veg == "-1"] <- NA 

dat$fresh_fruit <- as.numeric(dat$fresh_fruit)
dat$fresh_fruit[dat$fresh_fruit == "-10"] <- 0         
dat$fresh_fruit[dat$fresh_fruit == "-3"] <- NA         
dat$fresh_fruit[dat$fresh_fruit == "-1"] <- NA 

dat$dried_fruit <- as.numeric(dat$dried_fruit)
dat$dried_fruit[dat$dried_fruit == "-10"] <- 0         
dat$dried_fruit[dat$dried_fruit == "-3"] <- NA         
dat$dried_fruit[dat$dried_fruit == "-1"] <- NA 
...
dfSummary(dat$spread_type_details)
...
dat$bread[dat$bread == "-10"] <- 0        
dat$bread[dat$bread == "-3"] <- NA         
dat$bread[dat$bread == "-1"] <- NA  

dfSummary(dat$bread_type)
...
dat$cereal[dat$cereal == "-10"] <- 0        
dat$cereal[dat$cereal == "-3"] <- NA         
dat$cereal[dat$cereal == "-1"] <- NA  

dfSummary(dat$cereal_type)
dat$cereal_type <- factor(dat$cereal_type, ordered = FALSE)
dat$cereal_type <- relevel(dat$cereal_type, ref = "Oat cereal (e.g. Ready Brek, porridge)")

dat$salt <- factor(dat$salt, ordered = FALSE)
dat$salt <- relevel(dat$salt, ref = "Never/rarely")

dat$tea <- as.numeric(dat$tea)
dat$tea[dat$tea == "-10"] <- 0         
dat$tea[dat$tea == "-3"] <- NA         
dat$tea[dat$tea == "-1"] <- NA 

dfSummary(dat$coffee_type)
dat$coffee_type <- factor(dat$coffee_type, ordered = FALSE)
dat$coffee_type <- relevel(dat$coffee_type, ref = "Instant coffee")

dfSummary(dat$hot_drink_temp)
...
dat$water[dat$water == "-10"] <- 0         
dat$water[dat$water == "-3"] <- NA         
dat$water[dat$water == "-1"] <- NA 
...
dat$summer_outdoors_time[dat$summer_outdoors_time == "-10"] <- 0         
dat$summer_outdoors_time[dat$summer_outdoors_time == "-3"] <- NA         
dat$summer_outdoors_time[dat$summer_outdoors_time == "-1"] <- NA  

dat$winter_outdoors_time <- as.numeric(dat$winter_outdoors_time)
dat$winter_outdoors_time[dat$winter_outdoors_time == "-10"] <- 0         
dat$winter_outdoors_time[dat$winter_outdoors_time == "-3"] <- NA         
dat$winter_outdoors_time[dat$winter_outdoors_time == "-1"] <- NA  

dfSummary(dat$skin_color)
dat$skin_color <- factor(dat$skin_color, ordered = FALSE)
dat$skin_color <- relevel(dat$skin_color, ref = "Fair")

dfSummary(dat$skin_tan_ease)
...
dat$childhood_sunburn_number[dat$childhood_sunburn_number == "-3"] <- NA         
dat$childhood_sunburn_number[dat$childhood_sunburn_number == "-1"] <- NA  

dfSummary(dat$hair_color)
dat$hair_color <- factor(dat$hair_color, ordered = FALSE)
dat$hair_color <- relevel(dat$hair_color, ref = "Light brown")

dfSummary(dat$facial_aging)
...
dat$solarium_freq[dat$solarium_freq == "-10"] <- 0         
dat$solarium_freq[dat$solarium_freq == "-3"] <- NA         
dat$solarium_freq[dat$solarium_freq == "-1"] <- NA  
```
<br />   
...
dat$sex_age_first[dat$sex_age_first == "-3"] <- NA         
dat$sex_age_first[dat$sex_age_first == "-2"] <- NA         
dat$sex_age_first[dat$sex_age_first == "-1"] <- NA 

dat$number_sex_partners <- as.numeric(dat$number_sex_partners)
dat$number_sex_partners[dat$number_sex_partners == "-3"] <- NA         
dat$number_sex_partners[dat$number_sex_partners == "-1"] <- NA 
...
dat$number_same_sex_partners[dat$number_same_sex_partners == "-3"] <- NA         
dat$number_same_sex_partners[dat$number_same_sex_partners == "-1"] <- NA 
...
dat$fast_driving <- factor(dat$fast_driving, ordered = FALSE)
dat$fast_driving <- relevel(dat$fast_driving, ref = "Never/rarely")

dat$heavy_DIY_duration <- factor(dat$heavy_DIY_duration, ordered = FALSE)
dat$heavy_DIY_duration <- relevel(dat$heavy_DIY_duration, ref = "Less than 15 minutes")

dat$light_DIY_duration <- factor(dat$light_DIY_duration, ordered = FALSE)
dat$light_DIY_duration <- relevel(dat$light_DIY_duration, ref = "Less than 15 minutes")

dat$moderate_activity_duration <- as.numeric(dat$moderate_activity_duration)
dat$moderate_activity_duration[dat$moderate_activity_duration == "-3"] <- NA         
dat$moderate_activity_duration[dat$moderate_activity_duration == "-1"] <- NA 
...
dat$vigorous_activity_duration[dat$vigorous_activity_duration == "-3"] <- NA         
dat$vigorous_activity_duration[dat$vigorous_activity_duration == "-1"] <- NA 

dat$walks_duration <- as.numeric(dat$walks_duration)
dat$walks_duration[dat$walks_duration == "-3"] <- NA         
dat$walks_duration[dat$walks_duration == "-1"] <- NA 
...
dat$moderate_activity_over_10mins_days[dat$moderate_activity_over_10mins_days == "-3"] <- NA         
dat$moderate_activity_over_10mins_days[dat$moderate_activity_over_10mins_days == "-1"] <- NA 

dat$vigorous_activity_over_10mins_days <- as.numeric(dat$vigorous_activity_over_10mins_days)
dat$vigorous_activity_over_10mins_days[dat$vigorous_activity_over_10mins_days == "-3"] <- NA         
dat$vigorous_activity_over_10mins_days[dat$vigorous_activity_over_10mins_days == "-1"] <- NA 

dat$walked_over_10mins_days <- as.numeric(dat$walked_over_10mins_days)
dat$walked_over_10mins_days[dat$walked_over_10mins_days == "-3"] <- NA         
dat$walked_over_10mins_days[dat$walked_over_10mins_days == "-1"] <- NA 

dat$driving_time <- as.numeric(dat$driving_time)
dat$driving_time[dat$driving_time == "-10"] <- 0         
dat$driving_time[dat$driving_time == "-3"] <- NA         
dat$driving_time[dat$driving_time == "-1"] <- NA 

dat$computer_time <- as.numeric(dat$computer_time)
dat$computer_time[dat$computer_time == "-10"] <- 0         
dat$computer_time[dat$computer_time == "-3"] <- NA         
dat$computer_time[dat$computer_time == "-1"] <- NA 

dat$TV_time <- as.numeric(dat$TV_time)
dat$TV_time[dat$TV_time == "-10"] <- 0         
dat$TV_time[dat$TV_time == "-3"] <- NA         
dat$TV_time[dat$TV_time == "-1"] <- NA 
...
# remove columns used to make dummy vars
dat <- subset(dat, select = -c(physical_activity_type.0.0,
...
sort(unique(dat$diastolic_bp))
sort(unique(dat$systolic_bp))
sort(unique(dat$pulse_rate))

dat$diastolic_bp <- as.numeric(dat$diastolic_bp)
dat$systolic_bp <- as.numeric(dat$systolic_bp)
dat$pulse_rate <- as.numeric(dat$pulse_rate)

# remove excluded variables
dat <- subset(dat, select = -c(diastolic_bp_manual,
                               systolic_bp_manual,
                               pulse_rate_manual))
```
<br />   
...
# remove excluded variables
dat <- subset(dat, select = -c(pulse_rate_a_stiff))
...
# remove excluded variables
dat <- subset(dat, select = -c(weight_manual,
                               weight_impedance,
                               BMI_impedance,
                               seating_box_height,
...
# remove excluded variables
dat <- subset(dat, select = -c(blow_acceptability_text.0.0,
...
dfSummary(dat$bone_density_measure_foot)
...
dat$fractured_heel_left[dat$fractured_heel_left == "unsure"] <- NA         
...
dat$fractured_heel_right[dat$fractured_heel_right == "unsure"] <- NA         
...
# remove excluded variables
dat <- subset(dat, select = -c(heel_sound_speed_manual,
...
dfSummary(dat$home_population_density)
...
dat$maximum_digits_remembered[dat$maximum_digits_remembered == "-1"] <- NA 
...
dat$coffee[dat$coffee == "-10"] <- 0
dat$coffee[dat$coffee == "-3"] <- NA 
dat$coffee[dat$coffee == "-1"] <- NA 
...
dat$menopause_age[dat$menopause_age == "-3"] <- NA 
dat$menopause_age[dat$menopause_age == "-1"] <- NA 
...
# save final dataset
saveRDS(dat, file = "/Users/austin.argentieri/OneDrive - Nexus365/DPhil Research/UKB/Datasets/5. Recoded datasets/ukb_full_recoded_dataset.rds")
```

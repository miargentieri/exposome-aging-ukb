# XWAS of All-Cause Mortality - Results in full UK Biobank cohort (all sexes)
...
for(j in seq_along(discovery_combined)) {
    # divide survival time (days) by 365.25 to get time in years
    discovery_combined[[j]]$time <- 
        # change to numeric because survival time is a difftime object
        as.numeric(discovery_combined[[j]]$ACM_survival_time / 365.25) 
    
    # add time (years) to recruitment age to get censor age
    discovery_combined[[j]]$censor_age <-
        discovery_combined[[j]]$recruitment_age + discovery_combined[[j]]$time 
}

# create age at censoring variables for age-at-risk analysis - discovery
for(j in seq_along(replication_combined)) {
    # divide survival time (days) by 365.25 to get time in years
    replication_combined[[j]]$time <- 
        # change to numeric because survival time is a difftime object
        as.numeric(replication_combined[[j]]$ACM_survival_time / 365.25) 
    
    # add time (years) to recruitment age to get censor age
    replication_combined[[j]]$censor_age <-
        replication_combined[[j]]$recruitment_age + replication_combined[[j]]$time 
}
...
discovery_combined <- lapply(discovery_combined, split_birth_year)
replication_combined <- lapply(replication_combined, split_birth_year)
...
discovery_combined <- lapply(discovery_combined, scale_numeric)
replication_combined <- lapply(replication_combined, scale_numeric)
...
# run in datasets
discovery_combined <- lapply(discovery_combined, set_contrasts)
replication_combined <- lapply(replication_combined, set_contrasts)
...
model_disc <- function(x) {
  lapply(discovery_combined, function(y)
    coxph(as.formula(
      paste0(
...
model_rep <- function(x) {
  lapply(replication_combined, function(y)
    coxph(as.formula(
      paste0(
          "Surv(recruitment_age, censor_age, ACM_event_indicator) ~
             strata(birth_cohort, sex) + 
...
save(pool_disc,
     ...
     exposures,
     file = 
         paste0(paste0(paste0(path, "/results/full cohort/ACM_XWAS_results_"), date), "_all_sexes.RData")
)
...
manhattan <- 
  ...
  geom_point(
    ...
    ggtitle("XWAS of All-Cause Mortality among UK Biobank Participants") +
    labs(x = "Exposure",
         y = "-log10 p-value") +
...
ggsave(manhattan, 
       filename = 
           paste0(paste0(paste0(path, "/output/full cohort/ACM_XWAS_manhattan_"), date), "_all_sexes.png"),
       height = 7, width = 15)
...
fig <- ggplotly(manhattan, 
                tooltip = c("x", 
                            ...
                            "label", 
                            "text")) %>% 
  layout(title = "XWAS of All-Cause Mortality among UK Biobank Participants", 
         font = t)
...
saveWidget(
    fig,
    file = paste0(paste0(paste0(path, "/output/full cohort/html/ACM_XWAS_manhattan_"), date), "_all_sexes.html")
)
...
volc3 <- ggplotly(volc_loghr, 
                  tooltip = c("text",
                              ...
                              "label",
                              "label2")) %>% 
  layout(title = "XWAS of All-Cause Mortality among UK Biobank Participants", font = t ) %>% 
  layout(font=t)
...
write.csv(rep_table, file = paste0(paste0(paste0(path, "/output/full cohort/ACM_XWAS_output_"), date), "_all_sexes.csv"))
...
mcols <- unique(XWAS_total$Variable)
fcols <- unique(female_XWAS_total$Variable)
...
dict <- read.csv("/file/to/path/Argentieri UKB dataset data dictionary.csv")

# subset data dictionary to just variables used in imputation
dict <- dict[which(dict$Variable %in% total_cols), ]

# save 
write.csv(dict, "/file/to/path/XWAS_data_dictionary_mar_18_2022.csv")

...
# load telomere data
load(paste0(path, "/data/telomere_data_feb_01_2023.RData"))


# data <- read.csv(file = "/Users/austina/OneDrive - Nexus365/DPhil Research/UKB/Analysis projects/Proteomics/datasets/boruta_dataset_april_12_2023_full.csv")
...
save_as_docx(ftable, path = paste0(path, "/output/tables/table_1_feb_07_2023.docx"))
```
...
identify_prev_disease <- function(x) {
    ...
    # x$baseline_disease_indicator[which(x$hbA1c >= 48)] <- 1
    # x$baseline_disease_indicator[which(x$glucose >= 11.1)] <- 1
...
save_as_docx(ftable2, path = paste0(path, "/output/tables/table_S1_feb_07_2023.docx"))
```
...
save_as_docx(ftable3, path = paste0(path, "/output/tables/table_S4_feb_07_2023.docx"))
```
...
# nrow(data[which(!is.na(data$incident_colon_cancer)),])
...
# nrow(data[which(!is.na(data$incident_lung_cancer)),])
...
# nrow(data[which(!is.na(data$incident_eso_cancer)),])
...
# nrow(data[which(!is.na(data$incident_liver_cancer)),])
...
# nrow(data[which(!is.na(data$incident_pancreatic_cancer)),])
...
# nrow(data[which(!is.na(data$incident_brain_cancer)),])
...
# nrow(data[which(!is.na(data$incident_leukemia)),])
...
# nrow(data[which(!is.na(data$incident_lymphoma)),])
...
# nrow(data[which(!is.na(data$incident_breast_cancer)),])
...
# nrow(data[which(!is.na(data$incident_ovarian_cancer)),])
...
# nrow(data[which(!is.na(data$incident_prostate_cancer)),])
...
# nrow(data[which(!is.na(data$incident_diabetes)),])
...
# nrow(data[which(!is.na(data$incident_CVD)),])
...
# nrow(data[which(!is.na(data$incident_hypertension)),])
...
# nrow(data[which(!is.na(data$incident_cerebro)),])
...
# nrow(data[which(!is.na(data$incident_COPD)),])
...
# nrow(data[which(!is.na(data$incident_liver)),])
...
# nrow(data[which(!is.na(data$incident_kidney)),])
...
# nrow(data[which(!is.na(data$incident_all_dementia)),])
...
# nrow(data[which(!is.na(data$incident_vasc_dementia)),])
...
# nrow(data[which(!is.na(data$incident_alzheimers)),])
...
# nrow(data[which(!is.na(data$incident_parkinsons)),])
...
# nrow(data[which(!is.na(data$incident_rheumatoid)),])
...
# nrow(data[which(!is.na(data$incident_macular)),])
...
# nrow(data[which(!is.na(data$incident_osteoporosis)),])
...
# nrow(data[which(!is.na(data$incident_osteoarthritis)),])
...
save_as_docx(ftable4, path = paste0(path, "/output/tables/table_S2_feb_07_2023.docx"))
...
save_as_docx(ftable5, path = paste0(path, "/output/tables/table_S3_feb_07_2023.docx"))
...
data = merge(
    data,
    comb[which(colnames(comb) %nin% c("ACM_event_indicator", "ACM_survival_time", "recruitment_age", "sex"))],
    by = "eid",
    all.X = TRUE,
    all.y = FALSE,
    sort = FALSE
...
```{r descriptive_figure}

library(ggplot2)

# rm(all_data, scotwales, dat_NCD_all, dat_dx, comb)
# gc()
...
# change survival time to numeric
data$time <- as.numeric(data$ACM_survival_time / 365.25) 
# add time (years) to recruitment age to get censor age
data$censor_age <- data$recruitment_age + data$time

# subset to those who died
data2 <- data[which(data$ACM_event_indicator == "Dead"), ]


# density plot - age at death
mg1 <- ggplot(data2, aes(x = censor_age, fill = factor(sex, levels = c("Male", "Female")))) + 
    geom_density(aes(y = ..count..), alpha = 0.5, size = 1) +
    theme_classic() +
    ...
    theme(
        axis.title.y = element_text(size = 16, margin = margin(r = 20), face = "bold"),
...
# density plot - BMI
mg1a <- ggplot(data2, aes(x = BMI, fill = factor(sex, levels = c("Male", "Female")))) + 
    geom_density(aes(y = ..count..), alpha = 0.5, size = 1) +
    theme_classic() +
    scale_fill_brewer(palette = "Set1", direction = -1) +
    ylab("Mortality cases") +
...
### incident disease rates ----

ds_list <- c(
    Cs(
        incident_colon_cancer,
        incident_lung_cancer,
        incident_eso_cancer,
        incident_liver_cancer,
        incident_pancreatic_cancer,
        incident_brain_cancer,
        incident_leukemia,
        incident_lymphoma,
        incident_breast_cancer,
        incident_ovarian_cancer,
        incident_prostate_cancer,
        incident_diabetes,
        incident_CVD,
        incident_cerebro,
        incident_COPD,
...
table_list <- as.list(rep(1, length(ds_list)))

# get crosstab of no diagnosis, prevalent, incident for all diseases
for (k in seq_along(ds_list)) {
    table_list[[k]] <- as.data.frame(table(data[[ds_list[k]]]))
    table_list[[k]]$Disease <- disease_list2[k]
    colnames(table_list[[k]]) <- c("Var", "Freq", "Disease")
}

# rbind into single df
ds_table <- rbindlist(table_list)

# remove prevalent cases
ds_table2 <- ds_table[which(ds_table$Var != "No diagnosis"), ]

# remove rows with less than 10k cases
# ds_table3 <- ds_table2[which(ds_table2$Freq < 10000), ]
ds_table3 <- ds_table2[which(ds_table2$Freq < 2000), ]

# get only diseases where both prev and incident are < 10k
ds_table3 <- ds_table3[duplicated(ds_table3$Disease) | duplicated(ds_table3$Disease, fromLast = TRUE),]
ds_table3 <- ds_table3[which(ds_table3$Disease != "Osteoporosis"), ]

# get high and low prev diseases
low_prev <- unique(ds_table3$Disease)
high_prev <- disease_list2[disease_list2 %nin% low_prev]

# make group for high prev and low prev
ds_table2$group <- NA
ds_table2$group[which(ds_table2$Disease %in% low_prev)] <- 1
ds_table2$group[which(ds_table2$Disease %in% high_prev)] <- 2

# duplicate
ds_table3 <- ds_table2

# get sum across prev and incident cases
# library(dplyr)
# ds_table3 <- ds_table3 %>%
#   group_by(Disease) %>%
#   summarize(
...
ds_table3 <- aggregate(ds_table3$Freq, by=list(ds_table3$Disease), FUN=sum)

# order
ds_table3 <- ds_table3[order(ds_table3$x, decreasing = TRUE), ]
disease_order <- unique(ds_table3$Group.1)

# # get only diseases where both prev and incident are > 10k
# ds_table4 <- ds_table2[(ds_table2$Disease %nin% ds_table3$Disease), ]

# make factor
ds_table2$Disease <- factor(ds_table2$Disease, levels = disease_order)
ds_table2$group <- factor(ds_table2$group, levels = c(1,2))


# plot all
library(viridis)
mg2 <- ggplot(ds_table2, aes(x = Disease, y = Freq, fill = Var)) + 
    geom_bar(aes(fill = Var), position = "stack", stat = "identity") +
    theme_classic() +
    ylab("Disease cases") +
    xlab("") +
    scale_fill_manual(values = viridis(n = 3)[c(2:1)]) +
    scale_y_continuous(
        limits = c(0,100000), breaks = seq(0,1000000, by = 10000),
        labels = function(x) format(x, big.mark = ",", scientific = FALSE),
        expand = c(0.01, 0.05)
    ) +
    scale_x_discrete(expand = c(0.025, 0.01)) +
    theme(
        # axis.title.y = element_text(size = 16, margin = margin(r = 20), face = "bold"),
...
# plot all
mg2a <- ggplot(ds_table2, aes(x = Freq, y = Disease, fill = Var)) + 
    geom_bar(aes(fill = Var), position = "stack", stat = "identity") +
    theme_classic() +
    ylab("") +
    xlab("Disease cases") +
...
# plot those with small numbers
# mg3 <- ggplot(ds_table3, aes(x = Disease, y = Freq, fill = Var)) + 
#     geom_bar(aes(fill = Var), position = "stack", stat = "identity") +
#     theme_classic() +
#     ylab("Disease Cases") +
#     xlab("") +
#     scale_fill_manual(values = viridis(n = 3)[c(2:1)]) +
#     scale_y_continuous(
#         limits = c(0,10000), breaks = seq(0,100000, by = 1000),
#         labels = function(x) format(x, big.mark = ",", scientific = FALSE),
#         expand = c(0.01, 0.05)
#     ) +
#     scale_x_discrete(expand = c(0.04, 0.02)) +
#     theme(
#         axis.title.y = element_text(size = 16, margin = margin(r = 20), face = "bold"),
#         axis.text.x = element_text(angle = 45, hjust = 1, size = 11),
#         axis.text.y = element_text(size = 11),
...
# plot those with big numbers
# mg4 <- ggplot(ds_table4, aes(x = Disease, y = Freq, fill = Var)) + 
#     geom_bar(aes(fill = Var), position = "stack", stat = "identity") +
#     theme_classic() +
#     ylab("Disease Cases") +
#     xlab("") +
#     scale_fill_manual(values = viridis(n = 3)[c(2:1)]) +
#     scale_y_continuous(
#         limits = c(0,100000), breaks = seq(0,100000, by = 25000),
#         labels = function(x) format(x, big.mark = ",", scientific = FALSE),
#         expand = c(0.01, 0.05)
#     ) +
#     scale_x_discrete(expand = c(0.072, 0.02)) +
#     theme(
#         axis.title.y = element_text(size = 16, margin = margin(r = 20), face = "bold"),
#         axis.text.x = element_text(angle = 45, hjust = 1, size = 11),
#         axis.text.y = element_text(size = 11),
...
ds_table_list <- as.list(rep(1, length(fact_list)))
ds_table_list2 <- as.list(rep(1, length(fact_list)))

# get crosstab of no diagnosis, prevalent, incident for all diseases
for (k in seq_along(fact_list)) {
    ds_table_list[[k]] <- as.data.frame(table(data[[fact_list[k]]], data$ACM_event_indicator))
    ds_table_list[[k]]$Exposure <- fact_list[k]
    # colnames(ds_table_list[[k]]) <- c("Var", "Freq", "Disease")
}

for (k in seq_along(fact_list)) {
    ds_table_list2[[k]] <- as.data.frame(table(data[[fact_list[k]]], data$sex))
    ds_table_list2[[k]]$Exposure <- fact_list[k]
    # colnames(ds_table_list[[k]]) <- c("Var", "Freq", "Disease")
}


# rbind into single df
fact_table <- rbindlist(ds_table_list)
fact_table$Var1 <- gsub("Less than 18,000", "<18,000", fact_table$Var1)
fact_table$Var1 <- gsub("Greater than 100,000", ">100,000", fact_table$Var1)
fact_table$Var1 <- gsub("18,000 to 30,999", "18,000-30,999", fact_table$Var1)
fact_table$Var1 <- gsub("31,000 to 51,999", "31,000-51,999", fact_table$Var1)
fact_table$Var1 <- gsub("52,000 to 100,000", "52,000-100,000", fact_table$Var1)
fact_table$Var1 <- factor(fact_table$Var1, levels = unique(fact_table$Var1))

fact_table2 <- rbindlist(ds_table_list2)
fact_table2$Var1 <- gsub("Less than 18,000", "<18,000", fact_table2$Var1)
fact_table2$Var1 <- gsub("Greater than 100,000", ">100,000", fact_table2$Var1)
fact_table2$Var1 <- gsub("18,000 to 30,999", "18,000-30,999", fact_table2$Var1)
fact_table2$Var1 <- gsub("31,000 to 51,999", "31,000-51,999", fact_table2$Var1)
fact_table2$Var1 <- gsub("52,000 to 100,000", "52,000-100,000", fact_table2$Var1)
fact_table2$Var1 <- factor(fact_table2$Var1, levels = unique(fact_table2$Var1))
fact_table2$Var2 <- factor(fact_table2$Var2, levels = c("Male", "Female"))

# get percentages
fact_table3 <- transform(fact_table2, percent = ave(Freq, Exposure, FUN = prop.table))
fact_table3 <- fact_table2 %>% group_by(Exposure, Var2) %>% mutate(percent = (Freq/sum(Freq) * 100))
...
mg5 <- ggplot(fact_table, aes(y = Var1, x = Freq, fill = Var2)) +
    ...
    facet_grid(Exposure~., 
               scales = "free", 
               space = "free",
               labeller = labeller(Exposure = labels)
    ) +
    theme(
...
mg6 <- ggplot(fact_table3, aes(y = Var1, x = percent, fill = Var2)) +
    ...
    theme(
        ...
        legend.text = element_text(size = 11),
        legend.title = element_blank(),
        legend.position = c(1.17, 0.75),
        legend.box.background = element_blank(),
    ) +
    # coord_flip() +
    facet_grid2(Exposure~., 
                scales = "free", 
                space = "free",
                independent = "x",
                labeller = labeller(Exposure = labels)
...
fact_table4 <- fact_table3[which(fact_table3$Exposure %nin% c("population_density", "ethnicity")), ]
fact_table4$Var1 <- factor(fact_table4$Var1, levels = unique(fact_table4$Var1))
fact_table4$Exposure <- factor(fact_table4$Exposure, levels = unique(fact_table4$Exposure))
...
mg7 <- ggplot(fact_table4, aes(y = Var1, x = percent, fill = Var2)) +
    geom_bar(aes(fill = Var2), position = "dodge", stat = "identity") +
    scale_fill_brewer(palette = "Set1", direction = -1) +
    theme_classic() +
    xlab("") +
    ...
    facet_grid2(Exposure~., 
                ...
                space = "free",
                independent = "x",
                labeller = labeller(Exposure = labels2)
    ) +
    theme(
        strip.text.y = element_text(angle = 0, size = 11),
        strip.placement = "outside",
...
text4 <- "Percent of responses"
title4 <- text_grob(text4, 
                    just = c(0.3,0), 
                    color = "black", 
                    face = "bold", 
                    size = 16
...
plot_grid(NULL, mg1, NULL, NULL, mg2a, NULL, 
          NULL, title, NULL, NULL, title3, NULL,
          NULL, mg1a, NULL, NULL, mg7, NULL, 
          NULL, title2, NULL, NULL, title4, NULL,
          ncol = 6,
          nrow = 4,
          labels = c("a", "", "", "c", "", "",
                     "", "", "", "", "", "",
                     "b", "", "", "d", "",""),
          align = "hv",
          rel_widths = c(0.1, 1, 0.1, 0.1, 1, 0.1),

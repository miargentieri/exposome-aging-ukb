...
save(dat_accel, file = "./data/accelerometer_data.RData")
```

```{r accelerometer_QC}

# load accelerometer data
load("./data/accelerometer_data.RData")
...
# load XWAS results
load(paste0(path,"/results/full cohort/ACM_XWAS_results_aug_21_2022_all_sexes_summary.RData"))
...
# subset to sample with valid accelerometer data
# datasens <- lapply(all_data, function(x) {
#     x <- x[which(!is.na(x$overall_acceleration)), ]
#     return(x)
# })
...
### cannot compare with anova because Surv is different for the two models
AIC(fit[[1]], fit2[[1]])
...
# compare mortality rates in 2 populations
fit[[1]]$nevent/fit[[1]]$n
fit2[[1]]$nevent/fit2[[1]]$n

summary(all_data[[1]]$accel_survival_time)
summary(all_data[[1]]$ACM_survival_time)
...
cm <- c(
    'overall_acceleration' = 'Overall acceleration average',
...
library(tibble)
# make empty rows for factor reference levels
rows <- tribble(
...
# save
save_as_docx(accel_table, path = paste0(path, "/output/supplement/accelerometer_sensitivity_sept_01_2022.docx"))
```
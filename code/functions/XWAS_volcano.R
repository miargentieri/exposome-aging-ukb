        ...
        if (!is.null(sig.line)) {
            ...
            volcano <- ggplot2::ggplot(
                ...
                aes(x = .data[[x]], 
                    y = .data[[y]], 
                    label = .data[[exposure.label.col]],
                    text = paste(estimate.label, .data[[x]],
                                 "\n-log<sub>10</sub> p-value:", .data[[y]],
                                 "\nExposure:", .data[[exposure.label.col]]),
                    label2 = .data[[category]]
                    # text = paste(estimate.label, 
                    #              format(round(.data[[estimate.label.col]], 2), nsmall = 2))
                )) + 
    ...
    if (is.null(theme)) {
        ...
        if (!is.null(sig.line)) {
            ...
            volcano <- ggplot2::ggplot(
                ...
                aes(x = .data[[x]], 
                    y = .data[[y]], 
                    text = paste(estimate.label, format(round(.data[[x]], 2), nsmall = 2),
                                 "\n-log<sub>10</sub> p-value:", .data[[y]],
                                 "\nExposure:", .data[[exposure.label.col]],
        ...
        if (interactive == TRUE) {
            ...
            if (!is.null(category)) {
                ...
                fig <- plotly::ggplotly(
                    ...
                    layout(title = title, 
                           font = t) %>%
                    add_text(showlegend = show.legend)

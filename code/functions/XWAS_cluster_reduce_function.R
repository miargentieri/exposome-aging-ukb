...
list.of.packages <- c("Hmisc", "car", "survival", "mice", "broom")
...
cluster_reduce <- function(
    ...
    for (k in seq_along(clusters)) {
        ...
        if (!is.null(vars.list)) {
            ...
            vars[[k]] <- as.vector(unique(unlist(cluster.mat[clusters[k]])))
            ...
            while (i != 0) {
                ...
                df[[k]] <- tryCatch(as.data.frame(car::vif(fit)),
                                    error = function(e) {print(summary(fit))
                                        stop("Error in VIF. Check model summary above for NA estimates")})
                ...
                if (!is.null(vif.override)) {
                    index <- max(df[[k]][[vif.type]][which(df[[k]]$var %nin% vif.override)])
                } else {
    ...
    if (!is.null(vars.list)) {
        ...
        variables <- as.vector(unlist(cluster.mat))
    ...
    for (k in seq_along(sig_vars)) {
        ...
        if (is.list(pools[[k]]) == FALSE) {
            ...
            # make df summary of regression results
            if (multi.imputation == TRUE) {
                pool_df <- summary(pools[[k]])
            } else {
                pool_df <- tidy(pools[[k]])
            }
            ...
            if (!is.null(vars.list)) {
                ...
                # only remove covariates if they are not also in the cluster var list
                covar_remove <- covariates[covariates %nin% vars[[k]]]
                covar_remove <- paste(covar_remove, collapse = "|")
